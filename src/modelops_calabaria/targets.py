...
def wire_target_function_with_replicates(
    entrypoint: str,
    sim_returns: list,  # List of SimReturn objects
) -> Dict[str, Any]:
    """
    Wire function for evaluating targets on multiple replicates.

    This version receives individual replicate results, computes per-replicate
    losses, then aggregates. Useful for detailed diagnostics.
    ...
    entrypoint : str
        Target entrypoint in format "module.path:function_name"
    sim_returns : list
        List of SimReturn objects (one per replicate)

    Returns
    -------
    Dict[str, Any]
        Evaluation results including:
        - "total_loss": float (mean across replicates)
        - "target_losses": Dict[str, float] per-target losses
        - "per_replicate_losses": List[float] loss for each replicate
    """
    # Parse entrypoint
    if ":" not in entrypoint:
        raise ValueError(f"Invalid entrypoint format: {entrypoint}. Expected 'module:function'")
    ...
    # Convert each SimReturn's outputs to DataFrames
    replicate_dfs_list = []
    for sim_return in sim_returns:
        sim_output_dfs = {}
        for name, output_bytes in sim_return.outputs.items():
            if name != "metadata":
                sim_output_dfs[name] = pl.read_ipc(io.BytesIO(output_bytes))
        replicate_dfs_list.append(sim_output_dfs)

    # Evaluate targets on all replicates
    per_replicate_losses = []
    target_losses_by_replicate = {}

    for rep_idx, sim_output_dfs in enumerate(replicate_dfs_list):
        evaluations = targets.evaluate_all([sim_output_dfs])

        replicate_loss = 0.0
        for eval_result in evaluations:
            if eval_result.weighted_loss is not None:
                replicate_loss += eval_result.weighted_loss

                # Track per-target, per-replicate losses
                if eval_result.name not in target_losses_by_replicate:
                    target_losses_by_replicate[eval_result.name] = []
                target_losses_by_replicate[eval_result.name].append(eval_result.loss)

        per_replicate_losses.append(replicate_loss)

    # Aggregate across replicates (mean)
    total_loss = sum(per_replicate_losses) / len(per_replicate_losses) if per_replicate_losses else float('inf')

    # Mean per-target losses
    target_losses = {
        name: sum(losses) / len(losses)
        for name, losses in target_losses_by_replicate.items()
    }

    return {
        "total_loss": total_loss,
        "target_losses": target_losses,
        "per_replicate_losses": per_replicate_losses,
    }